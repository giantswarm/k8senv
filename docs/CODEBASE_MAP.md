---
last_mapped: 2026-02-18T18:30:00Z
total_files: 149
total_tokens: 375000
---
...
graph TB
    ...
    subgraph "Internal Core"
        ...
        Purge["core.purge — direct SQLite cleanup"]
    end
...
├── tests/purge/               # Purge strategy tests (ReleasePurge, direct SQLite cleanup)
...
│   │   ├── purge.go           # Direct SQLite cleanup: purgeViaSQL, bypass K8s API
...
| `strategy.go` | `ReleaseStrategy` type alias + 4 constants (Restart/Clean/Purge/None) | 250 |
...
| `purge.go` | Direct SQLite cleanup: purgeViaSQL, findUserNamespaces, deleteNamespaceData | 1200 |
...
const (
    ...
    ReleaseClean                           // Delete non-system namespaces via K8s API, keep running
    ReleaseNone                            // No cleanup, return as-is
    ReleasePurge                           // Direct SQLite DELETE, bypass K8s API, keep running
)
```
...
Release(token) → validate token (CAS) → switch strategy:
  ...
  ReleasePurge   → purgeViaSQL() → ReleaseFailed or ReleaseToPool
  ReleaseNone    → ReleaseToPool (no cleanup)
```

**purge.go** — Direct SQLite cleanup for `ReleasePurge`:
- Opens kine's `state.db` with WAL mode and busy_timeout
- Discovers non-system namespaces via SQL query + `isSystemNamespace()` filter
- Deletes all rows for user namespaces in a single transaction
- Pattern `%/<ns>/%` catches both core and group API resources
- Bypasses K8s admission chain (finalizers have no effect)
...
**`tests/purge/` (package `k8senv_purge_test`)** — SQLite purge with `ReleasePurge`:

| File | Tests | Purpose |
|------|-------|---------|
| `main_test.go` | -- | TestMain: singleton with `WithReleaseStrategy(ReleasePurge)` |
| `purge_test.go` | 6 | PurgeNamespaces, PreserveSystemNS, NoUserNS, Resources, Finalizers, SystemResources |

**`tests/stresspurge/` (package `k8senv_stresspurge_test`)** — Stress tests with `ReleasePurge`:

| File | Tests | Purpose |
|------|-------|---------|
| `main_test.go` | -- | TestMain: singleton with `WithReleaseStrategy(ReleasePurge)` |
| `stress_test.go` | 1 | Stress + verify clean instances via SQL purge |
...
2. **Phase 2**: Stress packages — `tests/stress`, `tests/stressclean`, `tests/stresspurge` sequentially
...
**Direct dependencies**: `gofrs/flock`, `modernc.org/sqlite` (pure Go, no CGO), `k8s.io/api`, `k8s.io/apiextensions-apiserver`, `k8s.io/apimachinery`, `k8s.io/client-go` (all v0.35.1), `golang.org/x/sync/errgroup`.
...
sequenceDiagram
    ...
    alt ReleaseRestart (default)
        ...
        Instance->>Instance: purgeViaSQL(sqlitePath)
        Instance->>Pool: ReleaseToPool (running)
    else ReleaseNone
...
16. **Stress tests run in separate phase** — `Makefile.test.mk` runs stress packages (`tests/stress`, `tests/stressclean`, `tests/stresspurge`) sequentially after all other packages to avoid resource exhaustion.

17. **ReleasePurge bypasses K8s API entirely** — Uses direct SQLite DELETEs via `modernc.org/sqlite` (pure Go). Finalizers, admission webhooks, and RBAC are all bypassed. Safe only because the instance is idle between Release and next Acquire, and `--watch-cache=false` ensures immediate consistency.
